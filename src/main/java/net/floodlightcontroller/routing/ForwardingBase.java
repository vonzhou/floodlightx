...
public abstract class ForwardingBase implements IOFMessageListener,
	 ...
	 */
	// 这个类会监听PACKET_IN消息 和 IDeviceManagerServices的通知
	public void startUp() {
	...
	public Command receive(IOFSwitch sw, OFMessage msg, FloodlightContext cntx) {
		...
		case PACKET_IN:
			...
			return this.processPacketInMessage(sw, (OFPacketIn) msg, decision,
					cntx);
			// 由具体实现类Forwarding来决策
		}
	 ...
	 * ports with inPort/outPorts passed in. Note that the packet in could be
	 ...
	 * @param cntx
	 */
	public void PacketOutMultiPort(OFPacketIn pi, IOFSwitch sw, short inPort,
			HashSet<Integer> outPorts, FloodlightContext cntx) {
		// setting actions
		List<OFAction> actions = new ArrayList<OFAction>();
		...
		po.setActionsLength((short) (OFActionOutput.MINIMUM_LENGTH * outPorts
				.size()));
	...
	protected boolean isInSwitchBroadcastCache(IOFSwitch sw, OFPacketIn pi,
			FloodlightContext cntx) {
		if (sw == null)
			return true;
		...
		// some FORWARD_OR_FLOOD packets are unicast with unknown destination
		// mac
		// if (eth.isBroadcast() || eth.isMulticast())
		return sw
				.updateBroadcastCache(new Long(eth.hashCode()), pi.getInPort());

		// return false;
	}

	public static boolean blockHost(
			IFloodlightProviderService floodlightProvider, SwitchPort sw_tup,
			long host_mac, short hardTimeout, long cookie) {
		...
		fm.setCookie(cookie).setHardTimeout((short) hardTimeout)
				.setIdleTimeout((short) 5)
				.setBufferId(OFPacketOut.BUFFER_ID_NONE).setMatch(match)
				.setActions(actions).setLengthU(OFFlowMod.MINIMUM_LENGTH); // +OFActionOutput.MINIMUM_LENGTH);
	...
	public void deviceMoved(IDevice device) {
		...
		OFMatch match = new OFMatch();
		match.setDataLayerDestination(Ethernet.toByteArray(device
				.getMACAddress()));
		...
		OFMessage fm = ((OFFlowMod) floodlightProvider.getOFMessageFactory()
				...
				.setMatch(match).setCookie(cookie)
